...
def get_event_data(ttree, event_num):
    ...
    # Number of photons for each track ID
    num_photons = np.array(ttree.parcount).astype(int)
    total_num_photons = int(ttree.n_photons_tot)
    ...
    secondary_data_dict = {
        ...
        "total_num_photons": total_num_photons,
    }
...
def group_data_by_track_id(ttree, secondary_data_dict):
    ...
    # Number of photons is already grouped by track ID, no extra action required
    grouped_num_photons = secondary_data_dict["num_photons"]
    total_num_photons = secondary_data_dict["total_num_photons"]
    ...
    grouped_secondary_data = {
        ...
        "num_photons": grouped_num_photons,
        "total_num_photons": total_num_photons,
    }
...
def get_secondary_info_for_process(process, grouped_secondary_data, energy_cut):

    """
    Extract sums of the secondary track lengths with and without the Frank-Tamm correction
    (see Eq. 6 in https://arxiv.org/pdf/1206.5530.pdf) and the numbers of generated Cherenkov photons.
    ...
    - num_photons_lowE_sec: number of photons generated by the secondaries with energies below energy_cut (int)
    - num_photons_muon: number of photons generated by the muon (int)
    ...
    if np.sum(mask) != 0:
        ...
        # Not all of the tracks are Cherenkov-generating. Select the ones that are:
        sel = (
            grouped_secondary_data["track_id"] - 1
            < len(grouped_secondary_data["num_photons"])
        ) & mask
        cherenkov_track_ids = grouped_secondary_data["track_id"][sel] - 1

        num_photons_for_track_ids = grouped_secondary_data["num_photons"][
            cherenkov_track_ids
        ]

        # Number of photons generated by the low-energy secondaries
        num_photons_lowE_sec = np.sum(num_photons_for_track_ids)
        # Number of photons generated by the muon
        num_photons_muon = grouped_secondary_data["num_photons"][0]
